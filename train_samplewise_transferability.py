...
def main():
    ...
    for epoch in range(1,args.epochs+1):
        ...
        if row["val_acc"]>best["val_acc"]:
            best=row.copy(); torch.save({"model":model.state_dict(),"discriminator":discriminator.state_dict(),"group":"SAMPLEWISE_TRANSFERABILITY","group_description":"DCRN + global Scene Shift alpha=0.8 + sample-wise affinity/reliability weighting","split_seed":1174,"optimization_seed":1174,"use_ilda":False,"use_scene_shift":True,"scene_shift_strength":0.8,"target_gt_used_for_training_or_selection":False,"discriminator":"spatial mean -> Linear(48,64) -> GELU -> Linear(64,1)","disabled_losses":["prototype","pseudo_label","LMMD","FixMatch","intra","inter","foundation","semantic","neighborhood","modulation"],"best":best},args.output/"best.pth")
