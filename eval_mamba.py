...
def main():
 ...
 for s in SPLITS:
  ck=torch.load(a.root/f'split_{s}'/'best.pth',map_location='cpu'); m=(SpectralSpatialGatedMambaClassifier() if ck.get('model_type')=='own' else MambaBackboneClassifier()).to(a.device); m.load_state_dict(ck['model']); m.eval(); x=patches(target,centers); pred=[]
  with torch.no_grad():
