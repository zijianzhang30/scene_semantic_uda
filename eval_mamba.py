...
from own_backbone import SpectralSpatialGatedMambaClassifier, SpectralPillarsClassifier
...
def main():
 ...
 for s in SPLITS:
  ck=torch.load(a.root/f'split_{s}'/'best.pth',map_location='cpu'); m=({'own': SpectralSpatialGatedMambaClassifier, 'pillars': SpectralPillarsClassifier}.get(ck.get('model_type'), MambaBackboneClassifier)()).to(a.device); m.load_state_dict(ck['model']); m.eval(); x=patches(target,centers); pred=[]
  with torch.no_grad():
