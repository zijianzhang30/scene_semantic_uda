...
def patches(cube,centers,w=12):
 h=w//2; p=np.pad(cube,((h,h),(h,h),(0,0))); o=np.empty((len(centers),cube.shape[-1],w,w),np.float32)
 for i,(r,c) in enumerate(centers): o[i]=p[r:r+w,c:c+w].transpose(2,0,1)
 return o
def main():
