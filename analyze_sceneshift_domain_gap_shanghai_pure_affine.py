"""Post-ILDA pure-affine SceneShift diagnostic for Shanghai -> Hangzhou.
...
def main():
    ...
    output = RUN / "seed_1341" / "domain_gap_analysis_pure_affine"
    ...
    with torch.no_grad():
        ...
        # Houston-matched pure affine transport: deliberately no random scale
        # and no smooth noise. Everything else matches the previous Shanghai
        # post-ILDA diagnostic.
    ...
    plot_tsne(
        ...
        "Shanghai->Hangzhou input spectral space (post-ILDA pure affine)",
    )
    plot_tsne(
