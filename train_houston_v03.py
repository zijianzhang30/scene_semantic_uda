...
def train(mode,epochs,out,warmup=None):
 ...
 for epoch in range(start_epoch,epochs+1):
  ...
  proto=valid=None; epoch_diag={'retained_ratio':0.0,'retained_membership_accuracy':0.0,'per_class_retained_target_count':[0]*K,'per_class_membership_mass':[0.0]*K,'per_class_ot_pair_count':[0]*K,'membership_max_mean':0.0,'membership_max_min':0.0,'membership_max_max':0.0,'membership_entropy':0.0,'tau_max':0.0}
  if mode!='source_only' and epoch>10:
   with torch.no_grad():
    ...
    with torch.no_grad(): ft=aux_features(model,pool); q=model.fc1(ft).softmax(-1); r=compute_soft_membership(ft,q,proto,valid_classes=valid); keep=uncertainty_filter(r,.9)
    rp=r.argmax(1); pool_y=ty[ids].to(DEVICE); epoch_diag.update({'retained_ratio':float(keep.float().mean()),'retained_membership_accuracy':float((rp[keep]==pool_y[keep]).float().mean()),'per_class_retained_target_count':torch.bincount(rp[keep],minlength=K).cpu().tolist(),'per_class_membership_mass':r.sum(0).cpu().tolist(),'membership_max_mean':float(r.max(1).values.mean()),'membership_max_min':float(r.max(1).values.min()),'membership_max_max':float(r.max(1).values.max()),'membership_entropy':float(-(r*r.clamp_min(1e-8).log()).sum(1).mean()),'tau_max':.3 if epoch<=20 else (.5 if epoch<=50 else .8)})
  ...
  for xb,yb in loader:
   ...
   if mode!='source_only' and epoch>10:
    with torch.no_grad(): ot=classwise_sinkhorn_ot(fs.detach(),yb,ft.detach(),r,K,filtered_mask=keep)
    ps,pt,pc=sample_ot_pairs(ot,fs,ft.detach(),32)
    epoch_diag['per_class_ot_pair_count']=[int(32 if c in ot else 0) for c in range(K)]
    if ps.numel():
     tau=.3 if epoch<=20 else (.5 if epoch<=50 else .8); lbr=bridge_classification_loss(model.fc1,ps,pt,pc,tau,flow_model=(flow if mode=='flow_bridge' else None));
     if mode=='flow_bridge': lfm=flow_matching_loss(flow,ps.detach(),pt.detach(),pc,tau)
