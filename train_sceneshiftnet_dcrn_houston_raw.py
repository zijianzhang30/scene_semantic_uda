...
if __name__=='__main__':
 p=argparse.ArgumentParser(); p.add_argument('--seed',type=int,default=1341); p.add_argument('--method',choices=['baseline','sceneshift'],default='sceneshift'); p.add_argument('--epochs',type=int,default=100); p.add_argument('--batch-size',type=int,default=32); p.add_argument('--source-per-class',type=int,default=180); p.add_argument('--use-ilda',type=parse_bool,default=False); p.add_argument('--normalization',choices=['none','loader'],default='none'); p.add_argument('--prepare-only',action='store_true'); a=p.parse_args(); run(a,SPEC,load_cubes,Path(__file__).parent/'runs_sceneshiftnet_dcrn/houston_raw',__file__)
