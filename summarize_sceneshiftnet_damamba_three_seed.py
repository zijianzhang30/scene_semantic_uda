...
ROOT = Path(__file__).resolve().parent / "runs_sceneshiftnet_damamba/houston_raw/three_seed"
METHODS = {
    "M0": "DAMamba baseline",
...
def main():
    ...
    lines = ["# Houston DAMamba backbone generalization", "",
    ...
    lines += ["", "## Paired changes", "",
              ...
              "", "## Interpretation", "",
              "Adaptive SceneShift lowers DAMamba OA in all three seeds under the strict raw M0 comparison, while improving AA in all three seeds. It changes the OA/AA tradeoff rather than producing a stable overall gain.",
              "",
              "Original MCC raises AA strongly at seed 1341, but lowers both OA and AA at seeds 2024 and 3407. Its weak-class benefit is therefore not stable on DAMamba under this protocol.",
              "",
              "The DAMamba result does not reproduce the DCRN pattern in which Adaptive SceneShift raised mean OA and MCC restored mean AA. This experiment therefore does not establish backbone-general effectiveness; it indicates substantial backbone and seed sensitivity.",
              "",
