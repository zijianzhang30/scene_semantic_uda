...
OUT=HERE/'runs_sceneshiftnet_damamba/houston_raw/three_seed_500epoch'
METHODS={
...
def run(args):
 spec=METHODS[args.method];seed_all(args.seed);suffix=f'seed_{args.seed}' if args.epochs==500 else f'seed_{args.seed}_test_{args.epochs}ep';out=OUT/args.method/suffix;out.mkdir(parents=True,exist_ok=False)
 ...
 config={
  ...
  'optimizer_note':'stable prior DAMamba Houston form retained; requested batch32/500 epochs retained',
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--method',choices=METHODS,required=True);p.add_argument('--seed',type=int,choices=[1341,2024,3407],required=True);p.add_argument('--epochs',type=int,default=500);run(p.parse_args())
