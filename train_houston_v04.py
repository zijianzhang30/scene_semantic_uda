 ...
 for epoch in range(start_epoch,epochs+1):
  ...
  for xb,yb in loader:
   ...
   if mode!='source_only' and epoch>10:
    ...
    safe=torch.tensor(rel,dtype=torch.bool,device=DEVICE)
    r_ot=r.clone(); r_ot[:,~safe]=0.0
...
def main():
 import argparse; p=argparse.ArgumentParser(); p.add_argument('--mode',choices=['warmup','source_only','linear_bridge','flow_bridge'],required=True); p.add_argument('--epochs',type=int,default=20); p.add_argument('--out',default=str(ROOT/'runs_v03')); p.add_argument('--seed',type=int,default=SEED); a=p.parse_args(); globals()['SEED']=a.seed; base=Path(a.out)
